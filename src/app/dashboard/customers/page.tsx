...
export default function CustomersPage() {
  ...
  const [targetPointCustomer, setTargetPointCustomer] = useState<Customer | null>(null);

  // Loyalty Settings Form State
  const [loyaltyForm, setLoyaltyForm] = useState<LoyaltySettings>(getLoyaltySettings());

  // Points Adjustment Form State
  const [adjustAmount, setAdjustAmount] = useState<string>("50");
  const [adjustType, setAdjustType] = useState<"EARNED" | "REDEEMED" | "BONUS" | "ADJUSTMENT">("BONUS");
  const [adjustReason, setAdjustReason] = useState<string>("Birthday / Promo Bonus");
  ...
  const [customerPointTx, setCustomerPointTx] = useState<PointTransaction[]>([]);
  ...
  const fetchCustomerOrders = useCallback(async (customerId: string) => {
    ...
    try {
      ...
      // Fetch Point Transactions Audit Log
  ...
  // ─── Loyalty Settings & Manual Points Handlers ─────────────────────────────
  const handleSaveLoyaltySettings = (e: React.FormEvent) => {
    e.preventDefault();
    saveLoyaltySettings(loyaltyForm);
    setIsSettingsOpen(false);
    alert("Loyalty Program Settings updated successfully!");
  };

  const handleOpenAdjustPoints = (cust: Customer) => {
    setTargetPointCustomer(cust);
    setAdjustAmount("50");
    setAdjustType("BONUS");
    setAdjustReason("Birthday / Special Bonus");
    setIsAdjustPointsOpen(true);
  };

  const handleApplyPointsAdjustment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!targetPointCustomer || !adjustAmount || isNaN(Number(adjustAmount))) return;

    const changeVal = parseInt(adjustAmount, 10);
    const finalChange = adjustType === "REDEEMED" ? -Math.abs(changeVal) : Math.abs(changeVal);

    setIsSubmitting(true);
    try {
      const ok = await recordPointTransaction(
        targetPointCustomer.id,
        finalChange,
        adjustType,
        adjustReason.trim() || `Manual adjustment (${adjustType})`
      );

      if (ok) {
        setIsAdjustPointsOpen(false);
        fetchCustomerData();
        if (viewCustomer && viewCustomer.id === targetPointCustomer.id) {
          handleOpenDetail(targetPointCustomer);
        }
      } else {
    ...
    <>
      ...
      <header className="border-b border-[#1d2434] pb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button
            onClick={() => {
              setLoyaltyForm(getLoyaltySettings());
              setIsSettingsOpen(true);
            }}
            variant="outline"
            className="border-amber-500/30 bg-amber-500/10 text-amber-300 hover:bg-amber-500/20 text-xs font-bold rounded-2xl h-11 px-4 flex items-center gap-2 cursor-pointer transition-all"
          >
          ...
          <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
            <DialogTrigger render={
              <Button className="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-slate-950 font-black shadow-lg shadow-emerald-500/20 text-xs rounded-2xl h-11 px-5 flex items-center gap-2 cursor-pointer transition-all">
                    ...
                    <tr key={cust.id} className="hover:bg-[#151b2a] transition-colors group">
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                          <Badge variant="outline" className={`text-[10px] font-bold px-2 py-0.5 ${getTierBadgeClass(cust.tier || "Bronze")}`}>
                            {cust.tier || "Bronze"}
                          </Badge>
                      ...
                      <td className="py-4 px-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <Button
                            ...
                            className="h-8 px-2 text-amber-400 hover:text-amber-300 hover:bg-amber-500/10 rounded-lg text-xs font-bold cursor-pointer flex items-center gap-1"
                            title="Gift or Adjust Reward Points"
                          >
                            <Gift className="w-3.5 h-3.5" /> Gift Pts
                          </Button>
      ...
      {/* Loyalty Rules Settings Modal */}
      <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
        <DialogContent className="bg-[#111520] border-[#1d2434] text-white rounded-3xl p-6 sm:max-w-lg">
          <DialogHeader>
          ...
          <form onSubmit={handleSaveLoyaltySettings} className="space-y-4 pt-3 text-xs">
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 space-y-3">
              <h4 className="font-bold text-amber-300 uppercase tracking-wider text-[11px]">Earning & Redemption Ratio</h4>
              
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-400 block mb-1">Spend Per 1 Point ($)</label>
                  <Input
                    type="number"
                    min={1}
                    value={loyaltyForm.dollarsPerPoint}
                    onChange={(e) => setLoyaltyForm({ ...loyaltyForm, dollarsPerPoint: Number(e.target.value) || 10 })}
                    className="bg-[#111520] border-slate-800 text-white h-9"
                  />
                  <span className="text-[10px] text-slate-500 mt-1 block">e.g. $10 spent = 1 pt</span>
                </div>

                <div>
                  <label className="text-slate-400 block mb-1">Points For Discount</label>
                  <Input
                    type="number"
                    min={10}
                    value={loyaltyForm.pointsPerDiscount}
                    onChange={(e) => setLoyaltyForm({ ...loyaltyForm, pointsPerDiscount: Number(e.target.value) || 100 })}
                    className="bg-[#111520] border-slate-800 text-white h-9"
                  />
                  <span className="text-[10px] text-slate-500 mt-1 block">e.g. 100 points needed</span>
                </div>
              </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setLoyaltyForm({ ...loyaltyForm, discountValue: Number(e.target.value) || 5 })}
                  className="bg-[#111520] border-slate-800 text-white h-9"
                />
                <span className="text-[10px] text-emerald-400 mt-1 block">
                  Current Rule: Every {loyaltyForm.pointsPerDiscount} pts = ${loyaltyForm.discountValue}.00 discount
                </span>
              </div>
            </div>

            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 space-y-3">
              <h4 className="font-bold text-amber-300 uppercase tracking-wider text-[11px]">VIP Tier Thresholds</h4>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-400 block mb-1">Silver Tier (Points Needed)</label>
                  <Input
                    type="number"
                    value={loyaltyForm.silverThreshold}
                    onChange={(e) => setLoyaltyForm({ ...loyaltyForm, silverThreshold: Number(e.target.value) || 500 })}
                    className="bg-[#111520] border-slate-800 text-white h-9"
                  />
                </div>

                <div>
                  <label className="text-slate-400 block mb-1">Gold Tier (Points Needed)</label>
                  <Input
                    type="number"
                    value={loyaltyForm.goldThreshold}
                    onChange={(e) => setLoyaltyForm({ ...loyaltyForm, goldThreshold: Number(e.target.value) || 1500 })}
                    className="bg-[#111520] border-slate-800 text-white h-9"
                  />
                </div>
              </div>
            ...
            <DialogFooter className="pt-2">
              <Button type="button" variant="outline" onClick={() => setIsSettingsOpen(false)} className="border-slate-800 text-slate-300">
                Cancel
              </Button>
      ...
      {/* Manual Gift / Adjust Points Modal */}
      {targetPointCustomer && (
        <Dialog open={isAdjustPointsOpen} onOpenChange={setIsAdjustPointsOpen}>
          <DialogContent className="bg-[#111520] border-[#1d2434] text-white rounded-3xl p-6 sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-xl font-extrabold text-white flex items-center gap-2">
                <Gift className="w-6 h-6 text-amber-400" />
                Gift / Adjust Points for {targetPointCustomer.fullName}
              </DialogTitle>
            </DialogHeader>

            <form onSubmit={handleApplyPointsAdjustment} className="space-y-4 pt-3 text-xs">
              <div className="bg-slate-950 p-3.5 rounded-2xl border border-slate-800 flex items-center justify-between">
                <span className="text-slate-400">Current Balance:</span>
                <span className="font-black text-amber-300 text-base">{targetPointCustomer.rewardPoints || 0} PTS</span>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-slate-800 text-white h-10 rounded-xl px-3 outline-none focus:border-amber-500"
                >
                  <option value="BONUS">🎁 Gift Bonus Points (+ Add)</option>
                  <option value="EARNED">⭐ Manual Earned (+ Add)</option>
                  <option value="REDEEMED">🔻 Deduct / Redeem Points (- Subtract)</option>
                  <option value="ADJUSTMENT">🔧 Administrative Adjustment (+ Add)</option>
                </select>
              </div>

              <div>
                <label className="text-slate-300 font-bold block mb-1">Points Amount</label>
                <Input
                  type="number"
                  min={1}
                  required
                  ...
                  placeholder="50"
                  className="bg-slate-950 border-slate-800 text-white h-10 rounded-xl font-bold text-sm"
                />
              </div>

              <div>
                <label className="text-slate-300 font-bold block mb-1">Reason / Description</label>
                <Input
                  required
                  value={adjustReason}
                  onChange={(e) => setAdjustReason(e.target.value)}
                  placeholder="e.g. Birthday Gift, Special Promotion, Support Resolution"
                  className="bg-slate-950 border-slate-800 text-white h-10 rounded-xl"
                />
              </div>

              <DialogFooter className="pt-2">
                <Button type="button" variant="outline" onClick={() => setIsAdjustPointsOpen(false)} className="border-slate-800 text-slate-300">
                  Cancel
                </Button>
                <Button type="submit" disabled={isSubmitting} className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold flex items-center gap-2">
          ...
          <div className="w-full max-w-xl bg-[#111520] border-l border-[#1d2434] h-full overflow-y-auto p-6 md:p-8 flex flex-col justify-between shadow-2xl">
            <div>
              ...
              <div className="grid grid-cols-3 gap-3 mt-6">
                <div className="bg-[#0a0d14] border border-[#1d2434] rounded-2xl p-3.5">
                ...
                <div className="bg-[#0a0d14] border border-amber-500/30 rounded-2xl p-3.5">
                  <span className="text-[10px] font-bold text-amber-400 uppercase tracking-wider block flex items-center gap-1">
                    <Sparkles className="w-3 h-3" /> Reward Points
                  </span>
                  <div className="flex items-baseline justify-between mt-1">
                    <span className="text-xl font-black text-amber-300">
                      {viewCustomer.rewardPoints || 0}
                    </span>
                    <Badge variant="outline" className={`text-[9px] font-bold px-1.5 py-0 ${getTierBadgeClass(viewCustomer.tier || "Bronze")}`}>
                      {viewCustomer.tier || "Bronze"}
                    </Badge>
                  </div>
                </div>
              </div>
              ...
              {/* Point Transactions History Timeline */}
              {customerPointTx.length > 0 && (
                <div className="mt-6 bg-[#0a0d14] border border-amber-500/20 rounded-2xl p-4 space-y-3">
                  <h4 className="text-xs font-black text-amber-300 uppercase tracking-wider flex items-center gap-1.5">
                    <Award className="w-4 h-4 text-amber-400" /> Reward Points History
                  </h4>
                  <div className="space-y-2 max-h-36 overflow-y-auto pr-1">
                    {customerPointTx.map((tx) => (
                      <div key={tx.id} className="flex items-center justify-between text-xs py-1.5 border-b border-[#1d2434] last:border-none">
                        <div>
                        ...
                        <span className={`font-black ${tx.pointsChange > 0 ? "text-emerald-400" : "text-rose-400"}`}>
